...
Overall status: **100% aligned for the compatible Hidemaru 9.57 chrome scope**.
...
|---|---|---|---|
| File | New/open/partial/binary, close-and-open, encoding reopen, reload, properties, rename, View Mode, edit-preserving overwrite prohibition, append save/read, print/page setup, complete save/close/discard variants, cursor-target opening, project folders, desktop/workspace save/open/history, WebKit browse mode, and suspendable file/folder/workspace history | ✅ Complete | All 42 official 9.57 File rows are mapped and placement-tested. Hidemaru Project/Desktop files map to folder projects and `.maruedit-workspace`; Quit/Close use macOS lifecycle conventions; FTP/SFTP/WebDAV files use Finder-mounted remote volumes and native Open/Save panels. |
...
1. ✅ **Complete** — Every compatible Hidemaru menu entry has a stable `CommandID`, enabled
...
- Gate 1 has a checked-in 336-row external Hidemaru inventory, exhaustive per-row
