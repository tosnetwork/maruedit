...
are not a parity measure. `ChromeParityAuditTests` instead requires every stable
ID to appear exactly once in the command reference and rejects unresolved
compatible rows in this matrix.
...
F-key activation, and user key bindings. `ChromeParityAuditTests` closes the
documentation gate.
