...
final class MaruCompatibilityTests: XCTestCase {
    func testEverySupportedPublicCommandBehavior() throws {
        ...
        var message = ""
        let translated = try MaruCompatibility.translate("message \"hello\";")
    ...
    func testUnsupportedSyntaxFailsExplicitly() {
        XCTAssertThrowsError(try MaruCompatibility.translate("run \"tool\";")) {
            XCTAssertEqual($0 as? MaruCompatibilityError, .unsafeCommand(line: 1, command: "run"))
        }
        XCTAssertThrowsError(try MaruCompatibility.translate("registry;")) {
            XCTAssertEqual($0 as? MaruCompatibilityError, .windowsOnlyCommand(line: 1, command: "registry"))
        }
        XCTAssertThrowsError(try MaruCompatibility.translate("#x = globalThis.secret;")) {
            guard case .invalidExpression = $0 as? MaruCompatibilityError else { return XCTFail("wrong diagnostic: \($0)") }
        }
    }
    ...
    func testPortableStatementsRouteThroughStableCommands() throws {
        ...
        _ = MacroEngine().execute(try MaruCompatibility.translate(
    ...
    func testRedistributableCorpusGeneratesPassingExecutableReport() {
        XCTAssertEqual(MaruCompatibilityCorpus.license, "CC0-1.0")
        let report = MaruCompatibilityCorpus.markdownReport()
        XCTAssertFalse(report.contains("FAIL"))
        for item in MaruCompatibilityCorpus.cases { XCTAssertTrue(report.contains(item.id)) }
    }

    func testFeatureFlagAndCatalogKeepNativeMacrosUnchanged() throws {
        XCTAssertFalse(MaruCompatibility.isEnabled(environment: [:], defaults: isolatedDefaults()))
        XCTAssertTrue(MaruCompatibility.isEnabled(
            environment: [MaruCompatibility.featureFlag: "1"], defaults: isolatedDefaults()))
        ...
        let enabled = MacroCatalogLoader.load(from: directory, enableMaruCompatibility: true)
    ...
    private func assertRunMany(_ source: String, initial: String, selections initialSelections: [NSRange],
        ...
        defer { text.deinitialize(count: 1); text.deallocate(); selections.deinitialize(count: 1); selections.deallocate() }
        var begin = 0, end = 0
        let result = MacroEngine().execute(try MaruCompatibility.translate(source), host: host(
    ...
    private func isolatedDefaults() -> UserDefaults {
        UserDefaults(suiteName: "MaruCompatibilityTests.\(UUID().uuidString)")!
    }
}
