...
final class OldMaruCompatibilityTests: XCTestCase {
    func testEverySupportedPublicCommandBehavior() throws {
        ...
        var message = ""
        let translated = try OldMaruCompatibility.translate("message \"hello\";")
    ...
    func testUnsupportedSyntaxFailsExplicitly() {
        XCTAssertThrowsError(try OldMaruCompatibility.translate("run \"tool\";")) {
            XCTAssertEqual($0 as? OldMaruCompatibilityError, .unsafeCommand(line: 1, command: "run"))
        }
        XCTAssertThrowsError(try OldMaruCompatibility.translate("registry;")) {
            XCTAssertEqual($0 as? OldMaruCompatibilityError, .windowsOnlyCommand(line: 1, command: "registry"))
        }
        XCTAssertThrowsError(try OldMaruCompatibility.translate("#x = globalThis.secret;")) {
            guard case .invalidExpression = $0 as? OldMaruCompatibilityError else { return XCTFail("wrong diagnostic: \($0)") }
        }
    }
    ...
    func testPortableStatementsRouteThroughStableCommands() throws {
        ...
        _ = MacroEngine().execute(try OldMaruCompatibility.translate(
    ...
    func testRedistributableCorpusGeneratesPassingExecutableReport() {
        XCTAssertEqual(OldMaruCompatibilityCorpus.license, "CC0-1.0")
        let report = OldMaruCompatibilityCorpus.markdownReport()
        XCTAssertFalse(report.contains("FAIL"))
        for item in OldMaruCompatibilityCorpus.cases { XCTAssertTrue(report.contains(item.id)) }
    }

    func testFeatureFlagAndCatalogKeepNativeMacrosUnchanged() throws {
        XCTAssertFalse(OldMaruCompatibility.isEnabled(environment: [:], defaults: isolatedDefaults()))
        XCTAssertTrue(OldMaruCompatibility.isEnabled(
            environment: [OldMaruCompatibility.featureFlag: "1"], defaults: isolatedDefaults()))
        ...
        let enabled = MacroCatalogLoader.load(from: directory, enableOldMaruCompatibility: true)
    ...
    private func assertRunMany(_ source: String, initial: String, selections initialSelections: [NSRange],
        ...
        defer { text.deinitialize(count: 1); text.deallocate(); selections.deinitialize(count: 1); selections.deallocate() }
        var begin = 0, end = 0
        let result = MacroEngine().execute(try OldMaruCompatibility.translate(source), host: host(
    ...
    private func isolatedDefaults() -> UserDefaults {
        UserDefaults(suiteName: "OldMaruCompatibilityTests.\(UUID().uuidString)")!
    }
}
