...
final class SyntaxHighlightCoordinatorTests: XCTestCase {
    ...
    func testInvalidRuleIsIgnoredWhileValidRulesStillMatch() async {
    ...
    func testVisibleRangeExpandsToBufferedWholeLinesAndClamps() async {
        let text = "zero\none\ntwo\nthree\n"
    ...
    func testNewRevisionPreventsQueuedStaleWorkFromApplying() async {
        ...
        let firstCompleted = LockedTestValue(false)
    ...
    func testDebounceOnlyAppliesNewestRequest() async {
    ...
    func testLargeFileModeDisablesRegexColorsWithoutChangingText() async {
    ...
    func testThemeRefreshChangesAttributesOnly() async {
        let storage = NSTextStorage(string: "let value = 1")
    ...
    func testPerformanceHighlightingRequiredViewportContext() async {
    ...
    func testRegexBudgetCapsInputMatchesAndPathologicalWork() async throws {
    ...
    func testStressFixtureSurvivesRapidRevisionChurn() async throws {
        let fixtureURL = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent().deletingLastPathComponent()
            .appendingPathComponent("Fixtures/Syntax/stress.swift")
        let fixture = try String(contentsOf: fixtureURL, encoding: .utf8)
        ...
        XCTAssertEqual(staleCompletions.value, 0)
